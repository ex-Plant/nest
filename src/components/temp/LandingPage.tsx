  ...
  return (
    <>
      <div className={`flex flex-col items-center md:hidden`}>
        <BirdLogo />
        <p className={`font-montclear tracking-[0.15em] text-3xl pt-4`}>{date}</p>
      </div>
      <div className={`hidden md:flex items-center justify-center gap-x-10 text-3xl`}>
        <p>Krzykawka</p>
        <BirdLogo />
        <p className={`font-montclear tracking-[0.15em]`}>{date}</p>
      </div>
    </>
  )
}
...
function BottomGroup() {
  return (
    <div className={`flex flex-col items-center gap-y-6`}>
      <p className={`font-montclear tracking-[0.15em] text-[1.25rem] md:hidden`}>{place}</p>
